...
if __name__=="__main__":
    ...
    parser.add_argument('-a',action='store',dest='arg_a',default=2.0,type=float)
    ...
    parser.add_argument('-s',action='store',dest='arg_sigma',default=0.2,type=float)
    result=parser.parse_args()
    def f(x,a=result.arg_a,b=result.arg_b,mu=result.arg_mu,sigma=result.arg_sigma):
        return k*math.sin(a*(x+b))+np.random.normal(mu,sigma)
    fi=open("train.txt","w")
